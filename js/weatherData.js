function weatherData(lat, lon, languageSelect) {
    $.ajax({
        ...
        data: {},
        url:
            "https://cors-anywhere.herokuapp.com/https://api.darksky.net/forecast/b1771da80a45a69cc2fcaf3cdbe9ab1a/" +
            lat +
            "," +
            lon +
            "?&units=auto&lang=" +
...
function editCards(days, data) {
    ...
    for (var i = 0; i < days.length; i++) {
        ...
        /* -- CLOUDY -- */
        if (
            days[i].icon === "partly-cloudy-day" ||
            days[i].icon === "partly-cloudy-night" ||
            days[i].icon === "cloudy"
        ) {
        ...
        /* -- SUNNY -- */
            days[i].icon === "clear-day" ||
            days[i].icon === "clear-night"
        ) {
        ...
        if (units == "us") {
            $("#forecastDay" + [i] + " .data .degrees").html(
                days[i].apparentTemperatureMax + "°F"
            );
        } else {
            $("#forecastDay" + [i] + " .data .degrees").html(
                days[i].apparentTemperatureMax + "°C"
            );
        }
        date.setDate(date.getDate() + 1);
        $("#forecastDay" + [i] + " .data .date").html(dateString);
        $("#forecastDay" + [i] + " .pullDown .summary").html(days[i].summary);
        $("#forecastDay" + [i] + " .pullDown .cloudCover").html(
            (days[i].cloudCover * 100).toFixed(2) + "%"
        );
        $("#forecastDay" + [i] + " .pullDown .windSpeed").html(
            days[i].windSpeed
        );
        $("#forecastDay" + [i] + " .pullDown .humidity").html(
            (days[i].humidity * 100).toFixed(2) + "%"
        );
        $("#forecastDay" + [i] + " .pullDown .dewPoint").html(
            days[i].dewPoint.toFixed(2)
        );
    }
...
function ChangeCityInput() {
    $("#city").html(
        "<input class='text-center' type='text' placeholder='Please enter a city' autofocus=''>"
    );
